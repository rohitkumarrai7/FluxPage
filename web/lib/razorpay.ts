...
export function getPlanAmount(tier: PaidTier): number {
  if (tier === "pro") {
    return parseInt(process.env.RAZORPAY_PLAN_PRO_AMOUNT || "19900", 10);
  }
  return parseInt(process.env.RAZORPAY_PLAN_PREMIUM_AMOUNT || "39900", 10);
}

/** Format paise as INR display string (e.g. 99900 → "₹999"). */
...
export function getPlanDisplayPrice(tier: PaidTier): string {
  if (tier === "pro") {
    return process.env.NEXT_PUBLIC_RAZORPAY_PRO_DISPLAY || formatPaiseAsInr(getPlanAmount("pro"));
  }
  return (
    process.env.NEXT_PUBLIC_RAZORPAY_PREMIUM_DISPLAY ||
    formatPaiseAsInr(getPlanAmount("premium"))
  );
}
