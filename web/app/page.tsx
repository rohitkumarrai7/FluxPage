...
function ProductMockup() {
  return (
    <div className="relative w-full max-w-xl mx-auto lg:max-w-none">
      <div className="absolute -inset-2 sm:-inset-4 bg-gradient-to-r from-primary/10 to-emerald-500/10 rounded-3xl blur-2xl" />
      <Card className="relative overflow-hidden shadow-card" padding="none">
        <div className="bg-slate-100 border-b border-border px-3 sm:px-4 py-2.5 flex items-center gap-2">
          ...
          <div className="flex-1 min-w-0 bg-white rounded-md px-2 sm:px-3 py-1 text-[10px] sm:text-xs text-muted truncate border border-border">
            linkedin.com/jobs/view/senior-software-engineer
          </div>
        </div>
        <div className="flex flex-col sm:flex-row">
          <div className="flex-1 p-4 sm:p-5 bg-white min-h-[200px] sm:min-h-[280px]">
          ...
          <div className="w-full sm:w-52 border-t sm:border-t-0 sm:border-l border-border bg-gradient-to-b from-slate-50 to-white p-4">
            <div className="flex sm:block items-center gap-3 sm:gap-0">
              <div className="text-xs font-semibold text-foreground sm:mb-3">Fluxpage Assistant</div>
              <div className="relative w-14 h-14 sm:w-16 sm:h-16 sm:mx-auto shrink-0">
                ...
                <span className="absolute inset-0 flex items-center justify-center text-base sm:text-lg font-black text-success">89</span>
              </div>
              <div className="flex-1 sm:text-center">
                <div className="text-xs text-muted sm:mb-3">ATS Match Score</div>
                <div className="flex flex-wrap gap-1 sm:justify-center">
                  {["React", "TypeScript", "Node.js"].map((k) => (
                    <span key={k} className="px-1.5 py-0.5 bg-emerald-50 text-success text-[10px] rounded-full font-medium">{k}</span>
                  ))}
                  {["AWS"].map((k) => (
                    <span key={k} className="px-1.5 py-0.5 bg-red-50 text-danger text-[10px] rounded-full font-medium">{k}</span>
                  ))}
                </div>
              </div>
            </div>
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-10 sm:pt-16 pb-12 sm:pb-20 grid lg:grid-cols-2 gap-8 lg:gap-12 items-center">
          <div className="text-center lg:text-left order-2 lg:order-1">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-50 border border-emerald-200 text-emerald-800 text-xs sm:text-sm mb-4 sm:mb-6">
            ...
            <p className="text-base sm:text-lg text-muted mb-6 sm:mb-8 leading-relaxed max-w-lg mx-auto lg:mx-0">
            ...
            <div className="flex flex-col sm:flex-row flex-wrap gap-3 justify-center lg:justify-start">
              <Button href={ctaHref} size="lg" className="w-full sm:w-auto">Get Started Free</Button>
              <Button href="#demo" variant="secondary" size="lg" className="w-full sm:w-auto">See It In Action</Button>
            </div>
      ...
      <section id="demo" className="max-w-5xl mx-auto px-4 sm:px-6 pb-20">
        ...
        <div className="flex justify-start sm:justify-center gap-2 mb-6 overflow-x-auto pb-1 -mx-1 px-1">
          {DEMO_TABS.map((tab) => (
            <button
              key={tab}
              onClick={() => setDemoTab(tab)}
              className={`shrink-0 px-4 py-2 rounded-full text-sm font-medium transition-colors ${
                demoTab === tab ? "bg-primary text-white shadow-sm" : "bg-slate-100 text-muted hover:text-foreground"
              }`}
            >
              {tab}
            </button>
